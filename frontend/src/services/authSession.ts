...
const PENDING_ANONYMOUS_MERGES_KEY = 'travel_pending_anonymous_merges'
...
export function stageCurrentAnonymousForMerge(): void {
  const principal = readAnonymousPrincipal(ANONYMOUS_PRINCIPAL_KEY)
  if (!principal) return
  const pending = readPendingAnonymousPrincipals()
  if (!pending.some((item) => item.access_token === principal.access_token)) {
    pending.push(principal)
    writePendingAnonymousPrincipals(pending)
  }
  clearAnonymousPrincipal()
}

export function getPendingAnonymousTokens(): string[] {
  return readPendingAnonymousPrincipals().map((principal) => principal.access_token)
}

export function clearMergedAnonymousToken(accessToken: string): void {
  writePendingAnonymousPrincipals(
    readPendingAnonymousPrincipals().filter((principal) => principal.access_token !== accessToken),
  )
}

export function hasPendingAnonymousMerge(): boolean {
  return readPendingAnonymousPrincipals().length > 0
}
...
function readPendingAnonymousPrincipals(): StoredAnonymousPrincipal[] {
  try {
    const raw = localStorage.getItem(PENDING_ANONYMOUS_MERGES_KEY)
    if (!raw) return []
...
function writePendingAnonymousPrincipals(principals: StoredAnonymousPrincipal[]): void {
  if (principals.length === 0) {
    localStorage.removeItem(PENDING_ANONYMOUS_MERGES_KEY)
    return
  }
  localStorage.setItem(PENDING_ANONYMOUS_MERGES_KEY, JSON.stringify(principals))
}
